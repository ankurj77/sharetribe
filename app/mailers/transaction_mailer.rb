      ...
      premailer_mail(:to => payment.recipient.confirmed_notification_emails_to,
        ...
        format.html {
          render "braintree_payment_receipt_to_seller", locals: {
                   ...
                   listing_price: APP_CONFIG.currency_symbol + humanized_money(payment.tx.unit_price),
                   listing_quantity: payment.tx.listing_quantity,
                   duration: duration,
                   payment_total: APP_CONFIG.currency_symbol + humanized_money(payment.total_sum),
                   payment_service_fee: APP_CONFIG.currency_symbol + humanized_money(-service_fee),
                   payment_seller_gets: APP_CONFIG.currency_symbol + humanized_money(you_get),
      ...
      premailer_mail(:to => payment.payer.confirmed_notification_emails_to,
        ...
        format.html {
          render "payment_receipt_to_buyer", locals: {
                   ...
                   duration: duration,
                   payment_total: APP_CONFIG.currency_symbol + humanized_money(payment.total_sum),
                   subtotal: APP_CONFIG.currency_symbol + humanized_money(payment.total_sum),
                   shipping_total: nil,
      ...
      premailer_mail(:to => seller_model.confirmed_notification_emails_to,
        ...
        format.html {
          render "paypal_payment_receipt_to_seller", locals: {
                   ...
                   listing_price: APP_CONFIG.currency_symbol + humanized_money(transaction[:listing_price]),
                   listing_quantity: transaction[:listing_quantity],
                   duration: transaction[:booking].present? ? transaction[:booking][:duration] : nil,
                   subtotal: APP_CONFIG.currency_symbol + humanized_money(transaction[:item_total]),
                   payment_total: APP_CONFIG.currency_symbol + humanized_money(payment_total),
                   shipping_total: APP_CONFIG.currency_symbol + humanized_money(transaction[:shipping_price]),
                   payment_service_fee: APP_CONFIG.currency_symbol + humanized_money(-service_fee),
                   paypal_gateway_fee: APP_CONFIG.currency_symbol + humanized_money(-gateway_fee),
                   payment_seller_gets: APP_CONFIG.currency_symbol + humanized_money(you_get),
      ...
      premailer_mail(:to => buyer_model.confirmed_notification_emails_to,
        ...
        format.html {
          render "payment_receipt_to_buyer", locals: {
                   ...
                   subtotal: APP_CONFIG.currency_symbol + humanized_money(transaction[:item_total]),
                   shipping_total: APP_CONFIG.currency_symbol + humanized_money(transaction[:shipping_price]),
                   payment_total: APP_CONFIG.currency_symbol + humanized_money(transaction[:payment_total]),
